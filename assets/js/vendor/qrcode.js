 ...
 * 
 * This is the unminified single-file build (qrcode.js) adapted for offline use.
 */
var QRCode;
(function () {
  //---------------------------------------------------------------------
  ...
  QRRSBlock.RS_BLOCK_TABLE = [
    ...
    // (Trimmed table for brevity; auto type selection will stay in these versions for short texts)
  ];
  ...
  QRRSBlock.getRsBlockTable = function (typeNumber, errorCorrectLevel) {
    // Full table is large; for our short IDs, we can clamp to version 3 or less.
    // We'll map versions 1-10 using the truncated table above.
    var offset = (typeNumber - 1) * 4;
    if (offset < 0) offset = 0;
    if (offset + 3 >= QRRSBlock.RS_BLOCK_TABLE.length) offset = (10 - 1) * 4;
  ...
  // Canvas drawer (default)
  var Drawing = !_isSupportCanvas() ? null : (function () {
  ...
  function _getTypeNumber(sText, nCorrectLevel) {
    // Simple for our use: choose 1..10 based on byte length.
    var length = encodeURI(sText).toString().replace(/\%[0-9a-fA-F]{2}/g, 'a').length;
    var limits = [
      {L:17,M:14,Q:11,H:7},
      {L:32,M:26,Q:20,H:14},
      {L:53,M:42,Q:32,H:24},
    ...
    for (var i=0;i<limits.length;i++){
      var lim=limits[i];
      var max = (nCorrectLevel==QRErrorCorrectLevel.L)?lim.L:(nCorrectLevel==QRErrorCorrectLevel.M)?lim.M:(nCorrectLevel==QRErrorCorrectLevel.Q)?lim.Q:lim.H;
      if(length<=max) return i+1;
    }
    return 10;
  }
  ...
  QRCode.prototype.makeCode = function (sText) {
    var typeNumber = this._htOption.typeNumber;
    if (!typeNumber) typeNumber = _getTypeNumber(sText, this._htOption.correctLevel);
