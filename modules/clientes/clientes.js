...
<script>window.focus();</script>
  ...
  async function onPrint(e){
    ...
    const clientId = (id.value||"").trim();
    const clientName = (name.value||"").trim();
    if(!clientId || !clientName){
      alert("Selecciona/guarda un cliente primero para imprimir su QR.");
      return;
    }
    const qrText = `DINAMITA:${clientId}`;
    const qrDataUrl = await window.dpMakeQrDataUrl(qrText, 180);
    const html = window.dpBuildCredencialHTML(clientName, qrDataUrl);
    window.dpPrintHTML(html, "Credencial");
  }
